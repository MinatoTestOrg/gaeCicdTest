...
export class GridComponent implements OnInit {
  ...
  initSelectAllActions() {
    if (this.gridConfig?.recordSelection != 'single') {
      $('.vs-grid-header th.select-checkbox').append('<input class="chk-select" type="checkbox" name="selectionCheckbox">')
    }
    let _this = this
  ...
  formatColumnDatas(col: any, data: any, type: any, row: any, meta: any, confRender: any = '') {
    ...
    switch (true) {
      ...
      case col.uiType === 'currency' && !confRender:
        formattedValue = row[col.field] ? this.getFormattedCurrency(data, row, col) : '';
        break;
      ...
      case col.uiType === 'number' && !confRender && typeof row[col.field] == "number":
        formattedValue = row[col.field] ? this.util.domSanitizer.sanitize(SecurityContext.HTML, self.util.getFormattedNumber(row[col.field], col)) : '';
        break;
  ...
  private attachSorting(dtInstance: any): void {
    ...
    headNode.on('click', (e: any) => {
      ...
      this.sortColumn = this.params.columns[(colOrder - 1)].data
  ...
  getFormattedCurrency(data: any, row: any, col: any) {
    ...
    return this.util.domSanitizer.sanitize(SecurityContext.HTML, self.util.formatCurrency(row[col.field], cCode, cDisplay, '', '', col));
  }
