...
export class ImportErrorDetailsComponent {
  @Input() errId:any;
  gridConfig: any = {};
  errData: any ={};
	quickFilter: any;
...
public appGlobalService = inject(AppGlobalService);
	...
	if (this.tableConfig.detailPage?.url) {
      const value: any = "parentId";
       let property: Exclude<keyof ImportErrorDetailsComponent, ''> = value;
       const methodName: any = "onUpdateChild";
       let action: Exclude<keyof ImportErrorDetailsComponent, ''> = methodName;
...
showToastMessage(config: object) {
    ...
    getErrorData(){
      ...
      const dataSubscription = this.importsService.getErrorData (params).subscribe((res:any) => {
          console.log(res)
          this.errData = res;
      });
    }
