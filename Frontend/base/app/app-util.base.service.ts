...
export class AppUtilBaseService {
  ...
  public formatDateTime(date: number, format?: string, locale?: string): string {
    format = format || BaseAppConstants.dateTimeFormatAngular;
    locale = locale || BaseAppConstants.defaultLocale;
  ...
  public formatNumber(value: number, digitsInfo: string, locale?: string): string {
    ...
    locale = locale || navigator.language || BaseAppConstants.defaultLocale;
  ...
  public getFormattedNumber(data: any, col: any): any { // Commented minInteger, thousandSeparator, decimalSeperator cause of primeNg not supporting this
    ...
    if (typeof col.maxFraction !== 'undefined' || typeof col.minFraction !== 'undefined' || col.uiType?.toLowerCase() == 'currency') {
      data = this._decimalPipe.transform(data, minInteger + '.' + minFraction + '-' + maxFraction)
    }
